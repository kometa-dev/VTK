...
void vtkCornerAnnotation::TextReplace(vtkImageActor* ia, vtkImageMapToWindowLevelColors* wl)
{
  ...
  if (wl)
  {
    ...
    {
      input_type_is_float =
        (wl_input->GetScalarType() == VTK_FLOAT || wl_input->GetScalarType() == VTK_DOUBLE);
    }
  }
  if (ia)
  {
    ...
    {
      input_type_is_float =
        (ia_input->GetScalarType() == VTK_FLOAT || ia_input->GetScalarType() == VTK_DOUBLE);
    }
  }
...
int vtkCornerAnnotation::RenderOpaqueGeometry(vtkViewport* viewport)
{
  ...
  else
  {
    ...
    for (int i = 0; i < numProps; i++)
    {
      ...
      if (ia)
      {
        if (ia->GetInput() && !wl)
        {
          wl = vtkImageMapToWindowLevelColors::SafeDownCast(ia->GetMapper()->GetInputAlgorithm());
        }
        break;
      }
    }
  ...
  int tprop_has_changed = (this->TextProperty && this->TextProperty->GetMTime() > this->BuildTime);
    ...
    if (viewport_size_has_changed || tprop_has_changed || this->GetMTime() > this->BuildTime)
    {
      ...
      if (tprop_has_changed)
      {
        for (int i = 0; i < NumTextPositions; i++)
        {
          ...
          tprop->SetFontSize(this->TextMapper[i]->GetTextProperty()->GetFontSize());
        }
...
void vtkCornerAnnotation::SetText(int i, const char* text)
{
  ...
  if (!text || (this->CornerText[i] && (!strcmp(this->CornerText[i], text))))
  {
    return;
  }
  delete[] this->CornerText[i];
